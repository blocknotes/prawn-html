...
RSpec.describe 'Headings' do
  ...
  context 'with some content in an element h1' do
    ...
    let(:expected_positions) do
      margin = PrawnHtml::Attributes.convert_size(PrawnHtml::Tags::H::MARGINS_TOP[:h1].to_s)
  ...
  context 'with some content in an element h2' do
    ...
    let(:expected_positions) do
      margin = PrawnHtml::Attributes.convert_size(PrawnHtml::Tags::H::MARGINS_TOP[:h2].to_s)
      y = pdf_doc.y - TestUtils.font_ascender(font_size: size) - margin
      [[pdf_doc.page.margins[:left], y.round(4)]]
    end
    let(:expected_font_settings) { [{ name: :'Helvetica-Bold', size: size }] }
  ...
  context 'with some content in an element h3' do
    ...
    let(:expected_positions) do
      margin = PrawnHtml::Attributes.convert_size(PrawnHtml::Tags::H::MARGINS_TOP[:h3].to_s)
      y = pdf_doc.y - TestUtils.font_ascender(font_size: size) - margin
      [[pdf_doc.page.margins[:left], y.round(4)]]
    end
    let(:expected_font_settings) { [{ name: :'Helvetica-Bold', size: size }] }
  ...
  context 'with some content in an element h4' do
    ...
    let(:expected_positions) do
      margin = PrawnHtml::Attributes.convert_size(PrawnHtml::Tags::H::MARGINS_TOP[:h4].to_s)
      y = pdf_doc.y - TestUtils.font_ascender(font_size: size) - margin
      [[pdf_doc.page.margins[:left], y.round(4)]]
    end
    let(:expected_font_settings) { [{ name: :'Helvetica-Bold', size: size }] }
  ...
  context 'with some content in an element h5' do
    ...
    let(:expected_positions) do
      ...
      margin = PrawnHtml::Attributes.convert_size(PrawnHtml::Tags::H::MARGINS_TOP[:h5].to_s)
  ...
  context 'with some content in an element h6' do
    ...
    let(:expected_positions) do
      ascender = TestUtils.font_ascender(font_family: 'Helvetica-Bold', font_size: size)
      margin = PrawnHtml::Attributes.convert_size(PrawnHtml::Tags::H::MARGINS_TOP[:h6].to_s)
      y = pdf_doc.y - ascender - margin
      [[pdf_doc.page.margins[:left], y.round(4)]]
    end
    let(:expected_font_settings) { [{ name: :'Helvetica-Bold', size: size }] }
